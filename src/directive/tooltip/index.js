...
var main = {
	render: function render() {
		...
		}, [_vm._v("\n      " + _vm._s(_vm.title) + "\n    ")]) : _vm._e(), _vm._v(" "), _vm.content ? _c('p', {
			staticClass: "v-tip-content",
			style: _vm.contentHeight
		}, [_vm._v("\n      " + _vm._s(_vm.content) + "\n    ")]) : _vm._e(), _vm._v(" "), _vm.customComponent ? _c(_vm.customComponent, _vm._g(_vm._b({
			tag: "component",
			on: {
				"hidden-tip": _vm.hiddenTip,
				"update-tip": _vm.updateTip
			}
		}, 'component', _vm.customProps, false), _vm.customListeners)) : _vm._e()], 1)]);
	},
	staticRenderFns: [],
	name: 'Vtip',
...
//			console.log(placementInfo,coordinate,this.placement,containerNode)
...
var directive = {
	install: function install(Vue$$1, options) {
		options = options || {};
		// 自定义指令的名字，默认为 tip
		var name = options.directiveName || 'tooltip';
		// tip 的展示方向
		...
		Vue$$1.directive(name, {
			bind: function bind(el, binding) {
				...
				el._tipHandler = function tipHandler() {
					...
					var tipOptions = (typeof options === 'undefined' ? 'undefined' : _typeof(options)) === 'object' ? Object.assign(mix, options, {
						target: this
					}) : Object.assign(mix, {
			...
			update: function update(el, _ref) {
				...
				if(el._tipInstance&&el._tipInstance.destroy){
					el._tipInstance.destroy();
				}
